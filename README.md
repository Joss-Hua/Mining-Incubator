...
Venus team have supported a distributed storage pool(mining pool) on Filecoin network based on Implementation-[Venus](https://venus.filecoin.io/Home.html), and the distributed mining architecture is designed for small and medium storage providers. Small and medium storage providers friendly from a distributed mining pool can be seen in different aspects, including but not limited to:
